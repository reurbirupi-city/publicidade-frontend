...
const ChatWhatsApp: React.FC<ChatWhatsAppProps> = ({
  ...
  const naoLidas = mensagens.filter(m => !m.lida && m.autor !== (isAdmin ? 'Admin' : 'Cliente')).length;
  ...
  return (
    <>
      ...
      {/* Janela do Chat */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-[9999] w-[380px] h-[500px] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200 dark:border-gray-700 animate-in slide-in-from-bottom-5 duration-300">
          ...
          <div className="bg-gradient-to-r from-green-600 to-green-700 dark:from-green-700 dark:to-green-800 px-4 py-3 flex items-center gap-3">
            <div className="relative">
              ...
              <span className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-green-600 ${
                statusDestinatario === 'online' ? 'bg-green-400' : 'bg-gray-400'
              }`} />
            </div>
            
            <div className="flex-1">
              <h3 className="font-semibold text-white text-sm">{nomeDestinatario}</h3>
              <p className="text-xs text-green-100">
                {statusDestinatario === 'online' ? 'Online' : 'Offline'}
              </p>
            </div>
            ...
            {mensagens.length === 0 ? (
              ...
              Object.entries(mensagensAgrupadas).map(([data, msgs]) => (
                <div key={data}>
                  ...
                  {/* Mensagens do dia */}
                  {msgs.map((msg) => {
                    const isMinhaMsg = isAdmin ? msg.autor === 'Admin' : msg.autor === 'Cliente';
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {/* Nome do autor (se não for minha mensagem) */}
                          {!isMinhaMsg && (

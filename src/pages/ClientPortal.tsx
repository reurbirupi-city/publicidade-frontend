...
const ClientPortal: React.FC = () => {
  ...
  // Estados para Feedback do Projeto
  ...
  // Função para enviar feedback do projeto
  const handleEnviarFeedback = async () => {
    if (!selectedProject || feedbackRating === 0) {
    ...
    try {
      const feedbackData = {
        ...
        projetoTitulo: selectedProject.titulo,
        clienteId: user?.uid,
        clienteNome: clientData?.nome || user?.email || 'Cliente',
        rating: feedbackRating,
        comentario: feedbackTexto.trim(),
        dataEnvio: new Date().toISOString(),
        adminId: clientData?.adminId || selectedProject.adminId || ''
      };

      // Salvar feedback na coleção feedbacks_projetos
      await addDoc(collection(db, 'feedbacks_projetos'), feedbackData);

      // Atualizar projeto com informação do feedback
      await updateDoc(doc(db, 'projetos', selectedProject.id), {
        feedbackCliente: {
          rating: feedbackRating,
          comentario: feedbackTexto.trim(),
          dataEnvio: new Date().toISOString()
  ...
  // Carregar feedback existente quando selecionar projeto
  useEffect(() => {
    if (selectedProject?.feedbackCliente) {
      setFeedbackRating(selectedProject.feedbackCliente.rating || 0);
      setFeedbackTexto(selectedProject.feedbackCliente.comentario || '');
      setFeedbackEnviado(true);
    } else {
      setFeedbackRating(0);
      setFeedbackTexto('');
              ...
              <div className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl p-6 border border-purple-200 dark:border-purple-700">
                ...
                {feedbackEnviado ? (
                  <div className="text-center py-4">
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-full mb-3">
                      <CheckCircle className="w-5 h-5" />
                      <span className="font-semibold">Feedback Enviado!</span>
                    </div>
                    <div className="flex justify-center gap-1 mb-2">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star
                          key={star}
                          className={`w-6 h-6 ${star <= feedbackRating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'}`}
                        />
                      ))}
                    </div>
                    {feedbackTexto && (
                      <p className="text-sm text-gray-600 dark:text-gray-400 italic">"{feedbackTexto}"</p>
                    )}
                  </div>
                ) : (
                  <>
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                      Sua opinião é muito importante! Avalie o projeto e deixe um comentário.
                    </p>
                    
                    {/* Estrelas de Rating */}
                    <div className="flex items-center gap-2 mb-4">
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Nota:</span>
                    ...
                    <div className="mb-4">
                      ...
                      <textarea
                        ...
                        placeholder="Conte-nos sobre sua experiência com este projeto..."
                        className="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white resize-none focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                        rows={3}
                      />
                    </div>
                    ...
                    <button
                      ...
                      disabled={feedbackRating === 0 || enviandoFeedback}
